...
//var_dump("SELECT * FROM comment where id_com = $id");
//die();
...
if ($_GET['action'] == 'save') {
    ...
    mysqli_query($db, "UPDATE `products` SET `name` = '$name', `description` = '$desc', `price` = '$price' WHERE `products`.`id` = $id");
}
...
//    var_dump("UPDATE comment SET name_com = '$nickname', com ='$text' WHERE id = $idprim");
//    die();
}
...
<a href="?action=edit&&id=<?= $row['id'] ?>">Изменить</a>
<a href="?action=delete&&id=<?= $row['id'] ?>">Удалить</a>
...
<?php foreach ($resultCom as $resultComs): ?>
    <h4>Пользователь:<?= $resultComs['name_com'] ?><a href="?com=edit&&idprim=<?=$resultComs['id']?>&&id=<?=$row['id']?>">[edit]</a><a href="?com=del&&idprim=<?=$resultComs['id']?>&&id=<?=$row['id']?>">[X]</a></h4>
